...
function MatchCard({
  ...
  const isWinnerA = !!winnerMint && winnerMint === coinAMint
  const isWinnerB = !!winnerMint && winnerMint === coinBMint
  const hasScores = (scoreA || 0) > 0 || (scoreB || 0) > 0
  const isLeadingA = !winnerMint && hasScores && (scoreA || 0) > (scoreB || 0)
  const isLeadingB = !winnerMint && hasScores && (scoreB || 0) > (scoreA || 0)

  const borderA = isWinnerA ? '2px solid #C8A84B' : selected === coinAMint ? '2px solid #00C41C' : isLeadingA ? '2px solid rgba(0,196,28,0.5)' : '2px solid transparent'
  const borderB = isWinnerB ? '2px solid #C8A84B' : selected === coinBMint ? '2px solid #00C41C' : isLeadingB ? '2px solid rgba(0,196,28,0.5)' : '2px solid transparent'
  const bgA = isWinnerA ? 'rgba(200,168,75,0.1)' : selected === coinAMint ? 'rgba(0,196,28,0.1)' : '#111'
  const bgB = isWinnerB ? 'rgba(200,168,75,0.1)' : selected === coinBMint ? 'rgba(0,196,28,0.1)' : '#111'
    ...
    <div style={{ background: '#0A0A0A', border: '1px solid rgba(0,196,28,0.2)', borderRadius: 12, padding: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        >
          ...
          {(scoreA || 0) > 0 && (
            <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>Score: <span style={{ color: '#00C41C', fontWeight: 700 }}>{(scoreA || 0).toFixed(1)}</span></div>
          )}
          {isWinnerA && <div style={{ color: '#C8A84B', fontSize: 11, fontWeight: 700 }}>WINNER 👑</div>}
          {isLeadingA && <div style={{ color: '#00C41C', fontSize: 11, fontWeight: 700 }}>LEADING 📈</div>}
          {selected === coinAMint && !isWinnerA && !isLeadingA && <div style={{ color: '#00C41C', fontSize: 11, fontWeight: 700 }}>SELECTED</div>}
          {selected === coinAMint && isLeadingA && <div style={{ color: '#00C41C', fontSize: 11, fontWeight: 700 }}>LEADING 📈 · SELECTED</div>}
        </button>
        ...
        >
          ...
          {(scoreB || 0) > 0 && (
            <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>Score: <span style={{ color: '#00C41C', fontWeight: 700 }}>{(scoreB || 0).toFixed(1)}</span></div>
          )}
          {isWinnerB && <div style={{ color: '#C8A84B', fontSize: 11, fontWeight: 700 }}>WINNER 👑</div>}
          {isLeadingB && <div style={{ color: '#00C41C', fontSize: 11, fontWeight: 700 }}>LEADING 📈</div>}
          {selected === coinBMint && !isWinnerB && !isLeadingB && <div style={{ color: '#00C41C', fontSize: 11, fontWeight: 700 }}>SELECTED</div>}
          {selected === coinBMint && isLeadingB && <div style={{ color: '#00C41C', fontSize: 11, fontWeight: 700 }}>LEADING 📈 · SELECTED</div>}
        </button>
      </div>
